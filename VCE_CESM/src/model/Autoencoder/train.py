...
if platform == 'win32': # su windows
    args = {}
    args['cfg_file'] = "./configs/autoencoder_train.yaml"
...
for fold in fold_list:
    ...
    # Train model
    model, history = util_model.train_model(model=model, criterion=criterion, optimizer=optimizer, scheduler=scheduler, model_name=model_name,
                                            data_loaders=data_loaders, model_dir=model_fold_dir, device=device, cfg_trainer=cfg['trainer'])
    ...
    mse_test_results, psnr_test_results, vif_test_results, ssim_test_results = util_model.test_4metrics(model, loader_train=False, loader_val=False, loader_test=data_loaders['test'],
