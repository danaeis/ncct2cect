...
if platform == 'win32': # su windows
    args = {}
    args['cfg_file'] = "./configs/cyclegan_tran_lear.yaml"
...
pretrained_path = os.path.join(cfg['data']['model_dir'], cfg['model']['pretrained_model']) # modello preaddestrato su dataset pubblico
...
util_general.create_dir(outputs_dir)  # output
...
for fold in fold_list:
    ...
    datasets = {step: util_data.ImgDataset(data=fold_data[step], cfg_data=cfg['data'], do_augmentation=True, step=step) for step in ['train', 'val', 'test']}
    ...
    opt_disc = optim.Adam(list(disc_REC.parameters()) + list(disc_LE.parameters()), lr=learning_rate, betas=(0.5, 0.999))
    ...
    # Carico modelli pre addestrati sul dataset pubblico
    ...
    CHECKPOINT_GEN_LE = os.path.join(pretrained_model_dir,cfg['trainer']['CHECKPOINT_GEN_LE'])
    CHECKPOINT_CRITIC_REC = os.path.join(pretrained_model_dir,cfg['trainer']['CHECKPOINT_CRITIC_REC'])
    CHECKPOINT_CRITIC_LE = os.path.join(pretrained_model_dir,cfg['trainer']['CHECKPOINT_CRITIC_LE'])

    load_checkpoint(CHECKPOINT_GEN_REC, gen_REC, opt_gen, learning_rate, device)
    load_checkpoint(CHECKPOINT_GEN_LE, gen_LE, opt_gen, learning_rate, device)
    load_checkpoint(CHECKPOINT_CRITIC_REC, disc_REC, opt_disc, learning_rate, device)
    load_checkpoint(CHECKPOINT_CRITIC_LE, disc_LE, opt_disc, learning_rate, device)

    # Train model --> Transfer learning su dataset FPUCBM
    ...
    mse_test_results, psnr_test_results, vif_test_results, ssim_test_results = util_model.test_4metrics(gen_REC, loader_train=False, loader_val=False, loader_test=data_loaders['test'],
                                                                                                        mse_metric=mse, psnr_metric=psnr, device=device, outputs_dir=outputs_fold_dir, save_output=True)
