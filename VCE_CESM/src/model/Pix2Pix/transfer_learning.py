...
if platform == 'win32': # su windows
    args = {}
    args['cfg_file'] = "./configs/pix2pix_tran_learn.yaml"
...
for fold in fold_list:
    ...
    CHECKPOINT_DISC = os.path.join(pretrained_model_dir, cfg['trainer']['CHECKPOINT_DISC'])


    load_checkpoint(CHECKPOINT_GEN, gen, opt_gen, learning_rate, device)
    load_checkpoint(CHECKPOINT_DISC, disc, opt_disc, learning_rate, device)

    # Train model --> Transfer learning su dataset FPUCBM
    gen, disc, history = util_model.train_pix2pix(gen=gen, disc=disc, data_loaders=data_loaders, early_stopping_criterion=early_stopping_criterion, opt_disc=opt_disc,
                                                  opt_gen=opt_gen, L1=L1_LOSS, bce=BCE, model_fold_dir=model_fold_dir, cfg_trainer=cfg['trainer'], device=device)
    ...
    mse_test_results, psnr_test_results, vif_test_results, ssim_test_results = util_model.test_4metrics(gen=gen, loader_train=False, loader_val=False, loader_test=data_loaders['test'],
